...
CREATE TABLE ADM_SERVICE(
    ...
    CONSTRAINT ADM_SERVICE_PK PRIMARY KEY (id)
);
...
CREATE TABLE ADM_COMPONENT_SERVICE(
    component_id INT NOT NULL,
    service_id INT NOT NULL,
    privileges VARCHAR(4) NOT NULL,
    CONSTRAINT ADM_COMPONENT_SERVICE_PK PRIMARY KEY (component_id, service_id)
);
...
CREATE TABLE MIN_MINISTRY_MEMBER(
	ministry_id INT NOT NULL,
	member_id INT NOT NULL,
	CONSTRAINT MIN_MINISTRY_MEMBER_PK PRIMARY KEY (ministry_id, member_id)
);
...
CREATE TABLE RTE_COLLABORATOR(
	...
	route_id INT NOT NULL,
	ministry_id INT,
    route_leader TINYINT(1) NOT NULL DEFAULT 0,
...
CREATE TABLE PMM_LESSON(
	...
	name VARCHAR(150),
	description VARCHAR(150),
	CONSTRAINT PMM_LESSON_PK PRIMARY KEY (discipleship_id, id)
);
...
ALTER TABLE MIN_MINISTRY_MEMBER ADD CONSTRAINT MIN_MINISTRY_MEMBER_MINISTRY_FK FOREIGN KEY (ministry_id) REFERENCES MIN_MINITRY(id) ON DELETE CASCADE ON UPDATE CASCADE;
ALTER TABLE MIN_MINISTRY_MEMBER ADD CONSTRAINT MIN_MINISTRY_MEMBER_MEMBER_FK FOREIGN KEY (member_id) REFERENCES PMM_MEMBER(person_id) ON DELETE CASCADE ON UPDATE CASCADE;
...
ALTER TABLE PMM_PERSON_DISCIPLESHIP ADD CONSTRAINT PMM_PERSON_DISCIPLESHIP_LESSON_FK FOREIGN KEY (discipleship_id, last_lesson_id) REFERENCES PMM_LESSON(discipleship_id, id) ON DELETE CASCADE ON UPDATE CASCADE;
...
ALTER TABLE ADM_COMPONENT ADD CONSTRAINT ADM_COMPONENT_NAME_UQ UNIQUE(name);
...
ALTER TABLE ADM_COMPONENT_SERVICE ADD CONSTRAINT ADM_COMPONENT_SERVICE_COMPONENT_FK FOREIGN KEY (component_id) REFERENCES ADM_COMPONENT(id) ON DELETE CASCADE ON UPDATE CASCADE;
ALTER TABLE ADM_COMPONENT_SERVICE ADD CONSTRAINT ADM_COMPONENT_SERVICE_SERVICE_FK FOREIGN KEY (service_id) REFERENCES ADM_SERVICE(id) ON DELETE CASCADE ON UPDATE CASCADE;
